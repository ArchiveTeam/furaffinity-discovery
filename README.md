...
### For Debian/Ubuntu:
    ...
    apt-get update && apt-get install -y git-core libgnutls-dev screen python-dev python-pip bzip2 zlib1g-dev
    pip install --upgrade seesaw
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/furaffinity-discovery.git; " archiveteam
    screen su -c "cd /home/archiveteam/furaffinity-discovery/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, furaffinity-discovery will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/furaffinity-discovery/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
